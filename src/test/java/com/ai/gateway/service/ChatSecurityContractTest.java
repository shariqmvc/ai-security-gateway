 ...
 * decision that can turn a valid tenant API-key request into an unexplained 403.
 */
class ChatSecurityContractTest {
    ...
    void chatIsHandledByApiKeyFilterAndNotSpringRoleAuthorization() throws Exception {
        Path config = Path.of(
                "src/main/java/com/ai/gateway/config/SecurityConfig.java"
        );

        String source = Files.readString(config);

        assertTrue(
                source.contains(".requestMatchers(\"/api/chat\").permitAll()"),
                "Chat must be delegated to the mandatory X-API-Key authentication filter."
        );

        Path filter = Path.of(
                "src/main/java/com/ai/gateway/authentication/AuthenticationFilter.java"
        );

        String filterSource = Files.readString(filter);

        assertFalse(
                filterSource.contains("servletPath.equals(\"/api/chat\")"),
